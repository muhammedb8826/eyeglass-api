...
- **Physically**, production is represented by moving each order item to **InProgress** (and then Ready / Delivered as you use them).
- **Before an item can go to InProgress:**
  1. **Operator stock** must exist for that item and be ≥ `orderItem.unit`, or the PATCH fails.
  2. **Lab tools** are only checked at **order create/update**, not again when moving to InProgress (see “Remaining” below).
...
3. For each order item, call **PATCH /api/v1/order-items/:id** and set **status** to **InProgress** when you start production (stock is then reduced and bincard recorded).
4. Move to **Ready** when the lens is made, then **Delivered** when handed to the customer (with payment check if `forcePayment` is on).

**Order delete:** An order can only be deleted when **all** its order items are in **Pending** status (order not yet in production).
